        		...
        		case 1:
        			...
        			case 1:
        				...
        				if(jointChoice.equalsIgnoreCase("y")) {
        					...
        					Application application = customer2.applyJointAccount(customer2);
        				}
        				else
        					customer.applyForAccount();
        				System.out.println("\tYour application has been subbmitted.");
        				System.out.print("\tTo log-out press enter, otherwise enter another menu option number: ");
        				String logOutChoice = scn.next();
        				if(logOutChoice.equals("\n"))
        					loggedin = false;
        				else
        		...
        		case 2:
        			...
        			if(checkWork.checkAdmin(username)) {
        				System.out.print("\n\tWithdraw from an account\t\t\t4\n\t"
        						...
        						+"Transfer between accounts\t\t\t6\n\tCancel and account\t\t\t\t7\n");
        			}
        			}
        			int empChoice = scn.nextInt();
        			switch(empChoice) {
        			case 1:
        				System.out.println("\tCustomer ID\t\t\tApplication Status\t\t\tJoint App\t\t\tApplication Number");
        			...
        			case 2:
        				...
        				for(int i = 0; i < allcusIds.size();i++)
        					System.out.println("\t"+ allcusIds.get(i) + "\t\t\t\t" + cusNames.get(i) + "\t\t\t\t"+ cusUsernames.get(i));
        				break;
        			case 3:
						...
						for(int i = 0; i < allcusIds2.size();i++)
        					System.out.println("\t"+ allcusIds2.get(i) + "\t\t\t\t" + accNames.get(i) + "\t\t\t\t"+ accNums.get(i)+ "\t\t\t\t"+ accBals.get(i));
        				break;
        			case 4:
        				System.out.println("\tEnter the account number you would like to withdraw from: ");
        				int accNum3 = scn.nextInt();
        			...
        			case 5:
        				...
        				if(checkAcc.checkAccNum(accNum4)) {
        					account3 = checkAcc.createAccObj(accNum4);
        					System.out.println("\tEnter the amount you would like to deposit: ");
        					double amount4 = scn.nextDouble();
        					account3.deposit(amount4);
        				}
        				else {
        					System.out.println("\tAccount not found");
        				    break;
